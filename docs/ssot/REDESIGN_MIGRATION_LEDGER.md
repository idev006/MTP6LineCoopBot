...
| Web frontend | direct fetches + client API key/mock fallbacks | shared session-authorized API clients + presentation-only UI | PARTIAL / MEMBER READS MIGRATED | member reads backend d78a1bc CI #87 + frontend 07ca08e CI #11 | migrate remaining write/admin/report/settings paths; remove remaining client API-key/mock callers |
